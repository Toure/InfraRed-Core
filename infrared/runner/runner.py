...
from infrared.core.logger import glob_logger
...
def ansible_playbook(playbook_path, verbose=2, settings=None,
    ...
    glob_logger.debug("Additional ansible args: {}".format(additional_args))
...
def _run_playbook(cli_args, settings):
    ...
    with tempfile.NamedTemporaryFile(
        ...
        glob_logger.debug('Starting ansible cli with args: {}'.format(cli_args[1:]))
        cli.parse()
        return cli.run()